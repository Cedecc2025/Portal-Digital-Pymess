...
function parseSession(rawValue, storageLabel) {
  if (!rawValue) {
    return null;
  }
  ...
  try {
    ...
    console.warn(`No fue posible leer la sesión almacenada en ${storageLabel}.`, error);
    return null;
  }
}
...
function getStoredSession() {
  ...
  const parsedLocal = parseSession(localSession, "localStorage");
  ...
  return parseSession(sessionSession, "sessionStorage");
}
